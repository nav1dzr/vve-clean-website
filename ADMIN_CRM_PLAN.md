...
Status: Phase 1 (foundation/auth/authorisation) **complete**. Phase 2 (dashboard, search, booking list, booking detail — read-only) **complete**. Phase 3 (internal notes, operational-status editing, balance tracking) **complete on this branch**.
...
> **Phase 3 implementation decisions** (internal notes, operational-status editing, balance tracking):
> - **Migration applied**: `20260719000000_create_internal_notes.sql` — `internal_notes(id, booking_id, author_admin_id, note, created_at)`, indexed on `(booking_id, created_at DESC)`, RLS enabled with zero anon/authenticated policies (same defence-in-depth pattern as `admin_users`). Append-only in code: `admin/api/bookings/[id]/notes.js` only exposes GET and POST, no PATCH/DELETE.
> - **Three new nested routes** under `admin/api/bookings/[id]/`: `notes.js` (GET list / POST append), `status.js` (PATCH, whitelisted against the 8 operational values, updates only `status`+`updated_at`), `balance.js` (PATCH, whitelisted against the 4 balance statuses + 5 payment methods, updates only `balance_status`+`balance_paid_at`+`balance_payment_method`+`updated_at` — no Stripe import exists in this file at all, verified by a test that reads the file's own source). A shared `admin/api/_lib/routeParams.js` (`extractIdParam(req, depthFromEnd)`) replaced the ad-hoc path-parsing that `bookings/[id].js` had grown in Phase 2, since three more nested routes now need the same logic at different path depths.
> - **Author identity**: `POST .../notes` always sets `author_admin_id` from the verified `auth.admin.id` returned by `verifyAdminRequest()` — any `author_admin_id`/`authorAdminId` field in the request body is read from nowhere in the handler and is silently ignored, not merely overwritten (see the "ignores any browser-supplied author id" test in `notes.test.js`).
> - **balance_paid_at / balance_payment_method clearing rule**: documented and enforced in `admin/api/bookings/[id]/balance.js` — both are set only when the new `balanceStatus` is `'paid'` (auto-timestamped with `now()` unless the caller supplies an explicit, validated ISO timestamp) and are unconditionally cleared to `null` on every other status. This was a required design decision (§4 of the Phase 3 spec asked for "an explicit and documented rule") rather than something the spec fully specified itself.
> - **Status-change confirmation**: implemented client-side only (`STATUS_VALUES_REQUIRING_CONFIRMATION` in `admin/src/lib/format.ts` — `cancelled` and `no_show`) via an inline `alertdialog`, not a native `window.confirm()`. The server route itself does not distinguish "confirmed" changes from routine ones — it has no concept of confirmation, by design, since that's a UI-layer safeguard against mis-clicks, not a security boundary.
> - **Card/list balance visibility**: `bookings.balance_status` was added to `CARD_SELECT` and `toCard()` (previously detail-only). Booking cards only render a balance badge when `balanceStatus` is non-null — omitted rather than showing "Balance unavailable" on every historical card, to avoid cluttering list views where most rows still have no balance data. The booking-list API also gained a whitelisted `balanceStatus` filter, reusing the existing filter/pagination architecture rather than a new endpoint.
> - **Modal accessibility bug found and fixed during Phase 3**: the reusable `Modal` component's focus-management `useEffect` originally depended on `[onClose]`, and `onClose` was a fresh function reference on every parent re-render — since the add-note form is a controlled `<textarea>`, every keystroke re-ran the effect and re-stole focus onto the modal's own close button (the first focusable element in DOM order), fighting the user's typing. Fixed with a ref-held `onClose` and a mount-only effect; the initial-focus query was also changed to prefer the first `input/textarea/select` over a bare `<button>`, so add-note correctly focuses the textarea rather than the ✕ close control. Caught by the "adds a note through the modal" test, not by inspection.
