...
export async function mediaCollectionHandler(req, res) {
  ...
  try {
    if (req.method === "GET")
      return await listMedia(req, res, headers, supabase);
    ...
    res.writeHead(500, headers);
    return res.end(
      JSON.stringify({ error: "Could not complete that media request." }),
    );
  }
}

async function listMedia(req, res, headers, supabase) {
  const page = Math.max(
    0,
    Math.min(10000, Number.parseInt(req.query?.page, 10) || 0),
  );
  const category = ["carpet", "sofa-upholstery", "end-of-tenancy"].includes(
    req.query?.category,
  )
    ? req.query.category
    : "";
  ...
  const [
    ...
    assetQuery.range(page * 30, page * 30 + 30),
    supabase
      .from(MEDIA_GALLERY_TOPICS_TABLE)
      .select("topic_key, label, description, sort_order")
      .eq("active", true)
      .order("sort_order"),
    supabase
      .from(MEDIA_GALLERY_SLOTS_TABLE)
      .select("id, topic_key, slot_code, slot_kind, label, sort_order")
      .order("topic_key")
      .order("sort_order"),
    supabase
      .from(MEDIA_WEBSITE_SLOTS_TABLE)
      .select(
        "id, slot_key, page_label, purpose_label, description, sort_order",
      )
      .order("sort_order"),
    supabase
      .from(MEDIA_ASSIGNMENTS_TABLE)
      .select(
        "id, gallery_slot_id, website_slot_id, asset_id, media_role, updated_at",
      ),
    supabase
      .from(MEDIA_REFERENCES_TABLE)
      .select(
        "id, reference_key, page_key, page_label, component_label, gallery_slot_id, website_slot_id, sort_order, active",
      )
      .eq("active", true)
      .order("page_key")
      .order("sort_order"),
  ...
  if (
    ...
    console.error(
      "[admin/media] list failed:",
      assetsError?.code ||
        topicsError?.code ||
        gallerySlotsError?.code ||
        websiteSlotsError?.code ||
        assignmentsError?.code ||
        referencesError?.code,
    );
    res.writeHead(500, headers);
    return res.end(
      JSON.stringify({
        error:
          "Could not load the two-part media manager. Make sure the latest Preview media migration is applied.",
      }),
    );
  }
  const pageAssets = (assets || []).slice(0, 30);
  const assignedIds = [
    ...new Set((assignments || []).map((row) => row.asset_id)),
  ].filter((id) => !pageAssets.some((row) => row.id === id));
  const { data: assignedAssets, error: assignedError } = assignedIds.length
    ? await supabase.from(MEDIA_ASSETS_TABLE).select("*").in("id", assignedIds)
    : { data: [], error: null };
  ...
  for (const reference of references || []) {
    ...
    current.push({
      ...
      pageKey: reference.page_key,
      pageLabel: reference.page_label,
      componentLabel: reference.component_label,
    });
  ...
  return res.end(
    JSON.stringify({
      assets: pageAssets.map((asset) =>
        toMediaSummary(asset, usagesByAsset.get(asset.id) || []),
      ),
      assignedAssets: (assignedAssets || []).map((asset) =>
        toMediaSummary(asset, usagesByAsset.get(asset.id) || []),
      ),
      nextPage: (assets || []).length > 30 ? page + 1 : null,
      ...
      gallerySlots: (gallerySlots || []).map((slot) => ({
        id: slot.id,
        topicKey: slot.topic_key,
        code: slot.slot_code,
        kind: slot.slot_kind,
        label: slot.label,
        sortOrder: slot.sort_order,
        assignments: (assignmentsByTarget.get(slot.id) || []).map(
          (assignment) => ({
            assetId: assignment.asset_id,
            role: assignment.media_role,
          }),
        ),
      ...
      websiteSlots: (websiteSlots || []).map((slot) => ({
        id: slot.id,
        key: slot.slot_key,
        pageLabel: slot.page_label,
        purposeLabel: slot.purpose_label,
      ...
      references: (references || []).map((reference) => ({
        ...
        pageLabel: reference.page_label,
        componentLabel: reference.component_label,
        gallerySlotId: reference.gallery_slot_id,
        websiteSlotId: reference.website_slot_id,
...
async function createUploadPlan(req, res, headers, supabase, adminId) {
  ...
  try {
    ...
    return res.end(
      JSON.stringify({ error: "The upload details were invalid." }),
    );
  }
  ...
  const mediaConfig = getMediaConfig(parsed.value.mediaType);
  if (!mediaConfig) {
    res.writeHead(503, headers);
    return res.end(
      JSON.stringify({
        error:
          "Media hosting is not configured yet. Add the media environment variables first.",
      }),
    );
  }
  ...
  if (error) {
    console.error(
      "[admin/media] asset create failed:",
      error.code,
      error.message,
    );
    res.writeHead(500, headers);
    return res.end(
      JSON.stringify({ error: "Could not create the media record." }),
    );
  }
  ...
  return res.end(
    JSON.stringify({ id: assetId, uploadUrl, expiresInSeconds: 900 }),
  );
}
