...
export const BEFORE_AFTER_VALUES = ["before", "after", "during", "none"];
export const PLACEMENT_VALUES = [
  "main-home",
  "gallery-end-of-tenancy",
  "gallery-carpet",
  "gallery-sofa",
  "carpet-page",
  "sofa-page",
...
export const validMediaUuid = (value) =>
  typeof value === "string" &&
...
export function validateNewAsset(body) {
  ...
  const mediaType = IMAGE_TYPES.has(contentType)
    ? "image"
  ...
  if (
    ...
    !mediaType ||
    !EXTENSIONS[contentType]?.includes(
      filename.split(".").pop().toLowerCase(),
    ) ||
    !Number.isSafeInteger(size) ||
    size < 1 ||
    size > MAX_SIZE_BYTES ||
    (mediaType === "image" && size > 40 * 1024 * 1024)
  ) {
    return {
      ok: false,
      error:
        "Choose a supported photo up to 40 MB or video up to 500 MB, with its original file extension.",
    };
  }
  return {
    ok: true,
    value: {
      ...
      size,
      mediaType,
      ...normaliseMetadata(body),
    },
  };
}

export function normaliseMetadata(body) {
  const placement = PLACEMENT_VALUES.includes(body.placement)
    ? body.placement
    : "gallery-end-of-tenancy";
  const defaults = PLACEMENT_DEFAULTS[placement];
  const category = CATEGORY_VALUES.includes(body.category)
    ? body.category
    : defaults.category;
  const beforeAfter = BEFORE_AFTER_VALUES.includes(body.beforeAfter)
    ? body.beforeAfter
    : "none";
  return {
    ...
    placement,
    beforeAfter,
    pairKey: text(body.pairKey, MAX_PAIR_KEY)
      .toLowerCase()
      .replace(/[^a-z0-9-]/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, ""),
...
export function toMediaSummary(row, usages = []) {
  return {
    ...
    usages,
    imageUrl:
      row.media_type === "image" && row.delivery_url
        ? row.delivery_url.replace("{width}", "768")
        : null,
