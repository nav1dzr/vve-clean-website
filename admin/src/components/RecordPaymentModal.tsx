...
interface Props {
  amountDue: number;
  onClose: () => void;
  onConfirm: (input: { amount: number; paymentDate: string; method: string; reference: string; notes: string }) => Promise<void>;
}
...
export default function RecordPaymentModal({ amountDue, onClose, onConfirm }: Props) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await onConfirm({ amount: numericAmount, paymentDate, method, reference: reference.trim(), notes: notes.trim() });
    } catch (err) {
