...
describe('private booking readiness', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('STRIPE_SECRET_KEY', 'private-stripe');
    ...
    vi.stubGlobal('fetch', vi.fn(async url => ({ ok: true, json: async () => url.endsWith('/account') ? mocks.account() : url.includes('/webhook_endpoints') ? mocks.hooks() : { ok: true } })));
  });
  ...
  it('checks connections without sending unless explicitly requested', async () => {
    ...
    expect(mocks.send).not.toHaveBeenCalled(); expect(fetch.mock.calls.at(-1)[0]).toContain('/getChat');
  });
  it('sends tests only to configured owner destinations', async () => {
    ...
    expect(JSON.parse(fetch.mock.calls.at(-1)[1].body).chat_id).toBe('owner-chat');
  });
