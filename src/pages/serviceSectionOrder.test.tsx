...
describe('Sofa & Upholstery page order', () => {
  it('runs hero → quote → results → extraction clip → reviews → benefits → process → gallery → includes → pricing → FAQs → related → CTA', () => {
    ...
    expectOrder([
      ...
      ['benefits', at.heading('What to expect from upholstery cleaning')],
  ...
  it('keeps the upholstery quote reachable and working', () => {
    ...
    expect(screen.getAllByRole('button', { name: /Request a time/i }).length).toBeGreaterThan(0);
  });
...
describe('Carpet page order', () => {
  it('runs hero → quote → results → reviews → benefits → process footage → includes → pricing', () => {
    ...
    expectOrder([
      ...
      ['reviews', at.sel('#reviews')],
      ['benefits', at.heading('What to expect from your carpet clean')],
