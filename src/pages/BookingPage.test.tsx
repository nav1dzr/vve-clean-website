...
function seedSelection(overrides: Partial<typeof VALID_SELECTION> = {}) {
  sessionStorage.setItem(
    "vve_booking",
    JSON.stringify({ ...VALID_SELECTION, ...overrides }),
  );
}
...
async function fillAllRequiredFields(user: ReturnType<typeof userEvent.setup>) {
  ...
  await user.selectOptions(
    screen.getByLabelText(/preferred arrival window/i),
    "Flexible",
  );
...
describe("BookingPage — mobile scheduling and access charges", () => {
  ...
  it("uses a one-column mobile schedule with full-width, clipping-safe controls and notes", () => {
    ...
    expect(screen.getByTestId("booking-schedule-fields")).toHaveClass(
      "grid-cols-1",
      "sm:grid-cols-2",
      "min-w-0",
    );
    expect(screen.getByLabelText(/preferred date/i)).toHaveClass(
      "w-full",
      "min-w-0",
      "max-w-full",
      "h-12",
    );
    expect(screen.getByLabelText(/preferred arrival window/i)).toHaveClass(
      "w-full",
      "min-w-0",
      "max-w-full",
      "h-12",
    ...
    expect(screen.getByLabelText(/anything else/i)).toHaveClass(
      "w-full",
      "min-w-0",
      "max-w-full",
    );
  ...
  it("requires both access questions before sending the request", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /^Send request — no payment$/ }),
    );
...
describe("BookingPage — booking request wording", () => {
  ...
  it("uses a request-first headline, not a guarantee claim", () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: "Request a preferred cleaning time",
      }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/slot is (nearly )?secured/i),
  ...
  it("shows the required supporting text near the date fields", () => {
    ...
    expect(
      screen.getByText(
        /This is a request, not a confirmed appointment, and no payment is taken at this stage\./,
      ),
    ).toBeInTheDocument();
  });
...
describe("BookingPage — accessible labels on property/contact fields", () => {
  ...
  it("associates the full name error with its input via aria-describedby when invalid", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /send request — no payment/i }),
    );
...
describe("BookingPage — required preferred date and arrival window", () => {
  ...
  it("blocks submission and shows an error when preferred date is missing", async () => {
    ...
    expect(
      await screen.findByText("Please choose your preferred date."),
    ).toBeInTheDocument();
  });
  ...
  it("blocks submission and shows an error when preferred arrival window is missing", async () => {
    ...
    expect(
      await screen.findByText("Please choose your preferred arrival window."),
    ).toBeInTheDocument();
  });
  ...
  it("blocks submission and shows an error when the typed date has already passed", async () => {
    ...
    expect(
      await screen.findByText(
        "Please choose a date that has not already passed.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("does not call the checkout API when required fields are missing", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Please choose your preferred date."),
      ).toBeInTheDocument();
    });
...
describe("BookingPage — no-payment request submission", () => {
  ...
  it("does not ask for payment or require booking terms before availability is checked", () => {
    ...
    expect(
      screen.getByText(
        /booking and cancellation terms apply once an appointment is confirmed/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("reuses its request key after a lost response and shows the saved server amount", async () => {
    const fetchMock = vi
      .fn()
    ...
    try {
      ...
      expect(await screen.findByText("VVE-REPLAY")).toBeInTheDocument();
      const bodies = fetchMock.mock.calls.map(([, init]) =>
        JSON.parse(init.body),
      );
      expect(bodies[0].requestKey).toMatch(/^[a-f0-9-]{36}$/i);
  ...
  it("keeps a saved request successful when browser draft cleanup is blocked", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue({
        ok: true,
        json: async () => ({ ok: true, bookingRef: "VVE-SAVED", total: 85 }),
      });
    vi.stubGlobal("fetch", fetchMock);
    ...
    try {
      ...
      expect(await screen.findByText("VVE-SAVED")).toBeInTheDocument();
      expect(screen.queryByRole("alert")).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /^Send request — no payment$/ }),
      ).not.toBeInTheDocument();
  ...
  it("offers a full refresh to review changed prices while preserving the form draft", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue({
        ok: false,
        ...
        json: async () => ({
          code: "price_changed",
          error: "Prices have changed. Please review your quote.",
          currentTotal: 95,
        }),
      });
    vi.stubGlobal("fetch", fetchMock);
    sessionStorage.setItem("vve_quote_origin", "/carpet-cleaning-london");
    try {
      const user = userEvent.setup();
      renderBookingPage();
      await fillAllRequiredFields(user);
      ...
      const refresh = screen.getByRole("link", {
        name: "Refresh and review quote",
      });
      expect(refresh).toHaveAttribute("href", "/carpet-cleaning-london#quote");
      refresh.addEventListener("click", (event) => event.preventDefault());
      await user.click(refresh);
      expect(sessionStorage.getItem("vve_restore_quote")).toBe("1");
      expect(
        JSON.parse(localStorage.getItem("vve_form_draft_v1")!).form,
      ).toMatchObject({
        fullName: "Jane Smith",
        date: FUTURE_DATE,
        time: "Flexible",
      });
      // Editing a contact detail must not silently accept the changed estimate.
      await user.type(screen.getByLabelText(/full name/i), " Jr");
      expect(
        screen.getByRole("button", { name: /^Send request — no payment$/ }),
      ).toBeDisabled();
      expect(
        screen.getByRole("link", { name: "Refresh and review quote" }),
      ).toBeInTheDocument();
      expect(fetchMock).toHaveBeenCalledTimes(1);
    } finally {
      vi.unstubAllGlobals();
