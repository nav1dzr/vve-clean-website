...
export type PublishedReference = {
  ...
  sort_order: number;
  source_type: "gallery" | "website";
  topic_key: string | null;
  slot_code: string;
  slot_kind: "before_after" | "video" | "photo" | "website";
  media_role: "before" | "after" | "primary";
...
const empty = (): Record<GalleryCategory, GalleryItem[]> => ({
  "end-of-tenancy": [],
  carpet: [],
  "sofa-upholstery": [],
});
let pending: Promise<PublishedReference[] | null> | null = null;
function fetchReferences() {
  if (!mediaSupabase) return Promise.resolve(null);
  if (pending) return pending;
  pending = (async () => {
    try {
      const { data, error } = await mediaSupabase.rpc(
        "public_media_references",
      );
      return !error && Array.isArray(data)
        ? (data as PublishedReference[])
        : null;
    } catch {
      return null;
    } finally {
...
function usePublishedReferences() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      if (live && data) setReferences(data);
    };
    void refresh();
    const onFocus = () => {
      void refresh();
...
export function referenceToItem(
  rows: PublishedReference[],
): GalleryItem | null {
  const first = rows[0];
  ...
  if (first.slot_kind === "before_after") {
    ...
    return {
      type: "before-after",
      id: `managed-${first.reference_key}`,
      label,
      before: mediaImageUrl(before.delivery_url, 1200),
      after: mediaImageUrl(after.delivery_url, 1200),
      beforeAlt: before.alt_text || `${label} before cleaning`,
      afterAlt: after.alt_text || `${label} after cleaning`,
    };
  }
  const row = rows.find((entry) => entry.media_role === "primary") || first;
  if (row.media_type === "image" && row.delivery_url)
    return {
      type: "photo",
      id: `managed-${row.reference_key}`,
      label,
      alt: row.alt_text || label,
      src: mediaImageUrl(row.delivery_url, 1200),
      srcSet: mediaImageSrcSet(row.delivery_url),
      sizes: managedImageSizes,
      fullSrc: mediaImageUrl(row.delivery_url, 2400),
    };
  if (row.media_type === "video" && row.mux_playback_id)
    return {
      type: "video",
      id: `managed-${row.reference_key}`,
      label,
