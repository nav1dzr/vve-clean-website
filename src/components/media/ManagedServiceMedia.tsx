...
export default function ManagedServiceMedia({
  pageKey,
  dark = false,
}: {
  pageKey: string;
  ...
  return (
    <div className="mb-10">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <MediaTile key={item.id} item={item} dark={dark} />
        ))}
      </div>
    </div>
  );
...
function MediaTile({ item, dark }: { item: GalleryItem; dark: boolean }) {
  ...
  if (item.type === "before-after")
    return <BeforeAfterTile entry={item} placeholderLabel={item.label} />;
  if (item.type === "video")
    return (
      <figure className={`overflow-hidden rounded-2xl ${background}`}>
        <ManagedVideo
          playerUrl={item.playerUrl || item.src}
          poster={item.poster}
          title={item.description || item.label}
        />
        <figcaption
          className={`p-3 text-sm font-semibold ${dark ? "text-white" : "text-navy-900"}`}
        >
          {item.label}
        </figcaption>
      </figure>
